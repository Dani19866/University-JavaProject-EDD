...
public class LinkedList<T> {
    ...
    public boolean searchUser(User user){
        ...
        while (aux != null){
            ...
            // ------------------------------
            if (compareer == user) returned = true;
            // ------------------------------
            aux = aux.getNextNode();
        }
    ...
    public int searchUserPosition(User user){
        int returned = -1;
        Node<T> aux = this.firstNode;
    ...
    public void helperShowUser() {
        try {
            ...
            while (aux != null) {
                User values = (User) aux.getData();
                System.out.println("ID: " + values.getId() + " || Username: " + values.getUser());

                aux = aux.getNextNode();
            }
        } catch (ClassCastException e) {
            Utils.error("No se puede invocar esta función, solo si el linkedList es de tipo: User", true);
        }
    ...
    public void helperShowRelations() {
        try {
            ...
            while (aux != null) {
                Relations values = (Relations) aux.getData();
                System.out.println("User: " + values.getUser1() + " || User: " + values.getUser2() + " || Time: " + values.getTime());
